...
package object microc {
  ...
  type Resp = (Stream[Valor], End, Mem) =>
    (Valor, Stream[Valor], List[String], End, Mem)
  ...
  def erro[T](msg: String): Acao[T] = k => sys.error(msg)

  def le(l: End): Acao[Valor] = k => (in, sp, mem) => mem.get(l) match {
    case Some(v) => k(v)(in, sp, mem)
    case None => sys.error("endereço " + l + " não existe")
  }

  def escreve(l: End, v: Valor): Acao[Valor] =
    k => (in, sp, mem) => k(v)(in, sp, mem + (l -> v))

  def bind[T, U](a: Acao[T], f: T => Acao[U]): Acao[U] =
    k => a(v => f(v)(k))

  def push(v: Valor): Acao[End] = k => (in, sp, mem) =>
    k(sp)(in, sp + 1, mem + (sp -> v))

  def pop(n: Int): Acao[Unit] = k => (in, sp, mem) => k(())(in, sp - n, mem)

  def print(vs: List[Valor]): Acao[Unit] = k => (in, sp, mem) => {
    val (v, nin, nout, nsp, nmem) = k(())(in, sp, mem)
    (v, nin, List(vs.map(v => v.toString()).mkString("\t")) ++ nout, nsp, nmem)
  }

  val read: Acao[Valor] = k => (in, sp, mem) => in.headOption match {
    case Some(v) => k(v)(in.tail, sp, mem)
    case None => sys.error("entrada está vazia")
  }
  ...
  def run(prog: Prog, in: Stream[Valor]) = {
    ...
    eval(Map(), corpo)(v => (in, sp, mem) => (v, in, List(), sp, mem))(in, 1000, Map(0 -> 1))
  }
}