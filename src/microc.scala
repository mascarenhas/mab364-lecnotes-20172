...
package object microc {
  ...
  // Ações primitivas
  type Acao[+T] = (End, Mem) => (Either[T, Erro], End, Mem)
  ...
  def lift[T](v: T): Acao[T] = (sp, mem) => (Left(v), sp, mem)

  def erro[T](msg: String): Acao[T] = (sp, mem) => (Right(Erro(msg)), sp, mem)

  def le(l: End): Acao[Valor] = (sp, mem) => mem.get(l) match {
    case Some(v) => (Left(v), sp, mem)
    case None => (Right(Erro("endereço " + l + " não existe")), sp, mem)
  }

  def escreve(l: End, v: Valor): Acao[Valor] =
    (sp, mem) => (Left(v), sp, mem + (l -> v))
  ...
  def bind[T, U](a: Acao[T], f: T => Acao[U]): Acao[U] =
    (sp, mem) => {
      ...
      v match {
        ...
        case Right(err) => (Right(err), nsp, nmem)
      }
    }

  def trycatch[T](atry: Acao[T], acatch: Acao[T]): Acao[T] =
    (sp, mem) => {
      val (v, nsp, nmem) = atry(sp, mem)
      v match {
        case Left(vv) => (Left(vv), nsp, nmem)
        case Right(err) => acatch(sp, nmem)
      }
    }

  def push(v: Valor): Acao[End] = (sp, mem) => (Left(sp), sp + 1, mem + (sp -> v))

  def pop(n: Int): Acao[Unit] = (sp, mem) => (Left(()), sp - n, mem)
