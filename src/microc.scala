...
package object microc {
  ...
  case class Transfer(exp: Exp) extends Exp
  ...
  type Resp = (Coroutines, Handlers, Stream[Valor], End, Mem) =>
  ...
  def erro[T](msg: String): Acao[T] = k =>
    (coros, handlers, in, sp, mem) => handlers match {
      case Handler(hk, hsp, hstk, hrest) => {
        val (vec, stk) = coros
        hk(())((vec, hstk), hrest, in, hsp, mem)
      }
      case EmptyH() => (Right(Erro(msg)), List())
    }

  val kid: Cont[Valor] = v => (coros, handlers, in, sp, mem) => (Left(v), List())

  // Try/Catch com pilha de tratadores
  def trycatch(atry: Acao[Valor], acatch: Acao[Valor]): Acao[Valor] =
    k => (coros, handlers, in, sp, mem) => {
      atry(v => (coros, handlers, in, sp, mem) =>
        k(v)(coros, handlers.tail, in, sp, mem))(coros, Handler(_ => acatch(k), sp, coros._2, handlers), in, sp, mem)
    }

  def le(l: End): Acao[Valor] = k => (coros, handlers, in, sp, mem) => mem.get(l) match {
    case Some(v) => k(v)(coros, handlers, in, sp, mem)
    case None => erro("endereço " + l + " não existe")(k)(coros, handlers, in, sp, mem)
  }

  def escreve(l: End, v: Valor): Acao[Valor] =
    k => (coros, handlers, in, sp, mem) => k(v)(coros, handlers, in, sp, mem + (l -> v))
  ...
  def push(v: Valor): Acao[End] = k => (coros, handlers, in, sp, mem) =>
    k(sp)(coros, handlers, in, sp + 1, mem + (sp -> v))

  def pop(n: Int): Acao[Unit] = k => (coros, handlers, in, sp, mem) => k(())(coros, handlers, in, sp - n, mem)

  def print(vs: List[Valor]): Acao[Unit] = k => (coros, handlers, in, sp, mem) => {
    val (v, nout) = k(())(coros, handlers, in, sp, mem)
    (v, vs.map(v => v.toString()).mkString("\t") :: nout)
  }

  val read: Acao[Valor] = k => (coros, handlers, in, sp, mem) => in.headOption match {
    case Some(v) => k(v)(coros, handlers, in.tail, sp, mem)
    case None => erro("entrada está vazia")(k)(coros, handlers, in, sp, mem)
  }
  ...
  def newcoro(corpo: Acao[Valor]): Acao[Valor] = k =>
    (coros, handlers, in, sp, mem) => {
      ...
      k(vec.length)((vec :+ coro, stk), handlers, in, sp, mem)
    }
  ...
  def resume(c: Valor): Acao[Valor] = k =>
    (coros, handlers, in, sp, mem) => {
      ...
      kc(0)((vec, Frame(k, sp, c, stk)), handlers, in, spc, mem)
    }

  def yieldc(v: Valor): Acao[Valor] = kc =>
    (coros, handlers, in, spc, mem) => coros match {
      case (vec, Frame(k, sp, c, next)) => k(v)((vec.updated(c, Coroutine(kc, spc)), next), handlers, in, sp, mem)
      case (vec, EmptyF()) => erro("yield fora de corotina")(kc)(coros, handlers, in, spc, mem)
    }
  ...
  def run(prog: Prog, in: Stream[Valor]) = {
    ...
    eval(Map(), corpo)(kid)((Vector(), EmptyF()), EmptyH(), in, 1000, Map(0 -> 1))
  }
}