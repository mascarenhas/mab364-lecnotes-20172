...
//      } yield es.foldRight(ce)({ case ((n, e), c) => Ap(Fun(List(n), c), List(e)) })
  ...
  def lift[T](v: T): Acao[T] = mem => (v, mem)

  def le(l: End): Acao[Valor] = mem => (mem(l), mem)

  def escreve(l: End, v: Valor): Acao[Valor] = mem => (v, mem + (l -> v))
  ...
  def run(prog: Prog) = {
    ...
    def eval(e: Exp, m: Mem): (Valor, Mem) = e match {
      ...
      case Seq(e1, e2) => eval(e1, m) match {
        case (Erro(msg), nm) => (Erro(msg), nm)
        case (_, nm) => eval(e2, nm)
      }
